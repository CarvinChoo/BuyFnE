...
function CreateAdminVoucherScreen(props) {
  ...
  const createNewVoucher = (voucher, resetForm) => {
    ...
    ref
      .set({
        ...
        category: Number(voucher.category.value),
        percent: percent,
      })
      .then(() => {
        Alert.alert("Success", "Created a new voucher for BuyFnE");
        resetForm();
        setLoading(false);
      })
      .catch((error) => {
...
export default CreateAdminVoucherScreen;
