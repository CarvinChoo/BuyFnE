...
function GroupBuyCheckoutScreen({ route, navigation }) {
  ...
  const createTransactionStatement = (charge_id) => {
    ...
    ref
      .set({
        ...
        discount: Number(
          (listing.price - listing.discountedPrice) * listing.count +
            listing.discountedPrice * listing.count * (loyaltyDiscount / 100)
        ),
        loyaltyApplied: Number(loyaltyDiscount),
        paid: Number(payableTotal),
        orderDate: timeNow,
    ...
    return (
      <View style={{ marginBottom: 20 }}>
        ...
        >
          ...
          <Text style={{ fontWeight: "bold", color: colors.chocolate }}>
        ...
        <View style={{ backgroundColor: colors.white, padding: 10 }}>
          ...
          >
            ...
            <Text style={{ color: colors.muted, fontSize: 15 }}>
              {"-$" +
                (orderTotal * (loyaltyDiscount / 100)).toFixed(2) +
                " ( " +
                loyaltyDiscount +
                "% )"}
            </Text>
          ...
          >
            <Text style={{ fontSize: 18 }}>Payable Total</Text>
            <Text style={{ color: colors.orangered, fontSize: 18 }}>
  ...
  const onChange = (number, item) => {
    setOrderTotal(Math.round(item.discountedPrice * number * 100) / 100);
    item.count = number;
    listing.count = number;
    var payable =
      Math.round(
        (item.discountedPrice * number -
          item.discountedPrice * number * (loyaltyDiscount / 100)) *
          100
      ) / 100;
    console.log(payable);
    setPayableTotal(payable);
